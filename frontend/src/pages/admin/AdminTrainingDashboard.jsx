import React, { useEffect, useState, useMemo, useCallback } from "react";
import { apiRequest } from "../../../utils/apiClient";
import { Link } from "react-router-dom";
import AdminLayout from "../../layouts/AdminLayout";
import { format } from "date-fns";
import { Eye, Clock, CheckCircle, Loader2 } from "lucide-react";

// Status Badge
const StatusBadge = ({ reviewed }) => (
  <span
    className={`inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-medium ${
      reviewed
        ? "bg-emerald-100 text-emerald-700"
        : "bg-amber-100 text-amber-700"
    }`}
  >
    {reviewed ? (
      <CheckCircle className="h-3 w-3" />
    ) : (
      <Clock className="h-3 w-3" />
    )}
    {reviewed ? "Reviewed" : "Pending"}
  </span>
);

export default function AdminTrainingDashboard() {
  const [apps, setApps] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  const [marking, setMarking] = useState({}); // { id: true }

  // Fetch all training applications
  useEffect(() => {
    (async () => {
      try {
        const res = await apiRequest("/trainings?skip=0&limit=100", "GET");
        setApps(Array.isArray(res) ? res : []);
      } catch (err) {
        setError(err.message || "Failed to load training applications");
      } finally {
        setLoading(false);
      }
    })();
  }, []);

  // Sort by newest first
  const sortedApps = useMemo(
    () =>
      [...apps].sort(
        (a, b) => new Date(b.created_at) - new Date(a.created_at)
      ),
    [apps]
  );

  // Mark as Reviewed
  const handleMarkReviewed = useCallback(async (id) => {
    setMarking((prev) => ({ ...prev, [id]: true }));
    try {
      await apiRequest(`/trainings/${id}/review`, "PUT", { reviewed: true });

      setApps((prev) =>
        prev.map((app) =>
          app.id === id ? { ...app, reviewed: true } : app
        )
      );
    } catch (err) {
      alert(`Failed to mark as reviewed: ${err.message}`);
    } finally {
      setMarking((prev) => ({ ...prev, [id]: false }));
    }
  }, []);

  // Loading State
  if (loading) {
    return (
      <AdminLayout>
        <div className="p-8 text-center">
          <Loader2 className="mx-auto h-8 w-8 animate-spin text-brand-600" />
          <p className="mt-2 text-slate-600">Loading training applications...</p>
        </div>
      </AdminLayout>
    );
  }

  // Error State
  if (error) {
    return (
      <AdminLayout>
        <div className="p-8 text-center">
          <p className="text-red-600">{error}</p>
          <button
            onClick={() => window.location.reload()}
            className="mt-4 text-sm text-brand-600 hover:underline"
          >
            Retry
          </button>
        </div>
      </AdminLayout>
    );
  }

  return (
    <AdminLayout>
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-slate-900">
          Training Applications
        </h1>
        <p className="mt-1 text-sm text-slate-600">
          {apps.length} total application{apps.length !== 1 ? "s" : ""}
        </p>
      </div>

      {/* Mobile: Card List */}
      <div className="space-y-4 md:hidden">
        {sortedApps.map((a) => {
          const data = a.data || {};
          return (
            <div
              key={a.id}
              className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm transition hover:shadow-md"
            >
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <p className="font-mono text-xs text-slate-500">#{a.id}</p>
                  <p className="mt-1 font-semibold text-slate-900">
                    {data.name || data.email || "Anonymous"}
                  </p>
                  <p className="mt-1 text-sm text-slate-600">
                    {format(new Date(a.created_at), "MMM d, yyyy")}
                  </p>
                  {data.thematicAreas && (
                    <p className="mt-1 text-xs text-brand-600">
                      {data.thematicAreas.length} thematic area{data.thematicAreas.length !== 1 ? "s" : ""}
                    </p>
                  )}
                </div>
                <StatusBadge reviewed={a.reviewed} />
              </div>

              <div className="mt-4 flex items-center justify-between gap-3">
                <Link
                  to={`/admin/training/${a.id}`}
                  className="flex items-center gap-1 text-brand-600 text-sm font-medium hover:underline"
                >
                  <Eye className="h-4 w-4" />
                  View
                </Link>

                {!a.reviewed && (
                  <button
                    onClick={() => handleMarkReviewed(a.id)}
                    disabled={marking[a.id]}
                    className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-white bg-emerald-600 rounded-md hover:bg-emerald-700 disabled:opacity-60 disabled:cursor-not-allowed transition"
                  >
                    {marking[a.id] ? (
                      <>
                        <Loader2 className="h-3 w-3 animate-spin" />
                        Saving...
                      </>
                    ) : (
                      <>
                        <CheckCircle className="h-3 w-3" />
                        Mark Reviewed
                      </>
                    )}
                  </button>
                )}
              </div>
            </div>
          );
        })}
      </div>

      {/* Desktop: Table */}
      <div className="hidden md:block overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
        <table className="min-w-full divide-y divide-slate-200">
          <thead className="bg-slate-50">
            <tr>
              <th className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-slate-500">
                ID
              </th>
              <th className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-slate-500">
                Applicant
              </th>
              <th className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-slate-500">
                Submitted
              </th>
              <th className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-slate-500">
                Thematic Areas
              </th>
              <th className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-slate-500">
                Status
              </th>
              <th className="px-6 py-3 text-right text-xs font-medium uppercase tracking-wider text-slate-500">
                Actions
              </th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-200 bg-white">
            {sortedApps.map((a) => {
              const data = a.data || {};
              return (
                <tr key={a.id} className="hover:bg-slate-50 transition">
                  <td className="whitespace-nowrap px-6 py-4 font-mono text-sm text-slate-900">
                    #{a.id}
                  </td>
                  <td className="px-6 py-4 text-sm">
                    <div>
                      <p className="font-medium text-slate-900">
                        {data.name || "—"}
                      </p>
                      <p className="text-slate-500 text-xs">
                        {data.email || "—"}
                      </p>
                    </div>
                  </td>
                  <td className="whitespace-nowrap px-6 py-4 text-sm text-slate-600">
                    {format(new Date(a.created_at), "MMM d, yyyy")}
                  </td>
                  <td className="px-6 py-4 text-sm text-slate-600">
                    {data.thematicAreas?.length > 0
                      ? `${data.thematicAreas.length} selected`
                      : "—"}
                  </td>
                  <td className="px-6 py-4">
                    <StatusBadge reviewed={a.reviewed} />
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex items-center justify-end gap-3">
                      {!a.reviewed && (
                        <button
                          onClick={() => handleMarkReviewed(a.id)}
                          disabled={marking[a.id]}
                          className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-white bg-emerald-600 rounded-md hover:bg-emerald-700 disabled:opacity-60 disabled:cursor-not-allowed transition"
                        >
                          {marking[a.id] ? (
                            <>
                              <Loader2 className="h-3 w-3 animate-spin" />
                              Saving
                            </>
                          ) : (
                            <>
                              <CheckCircle className="h-3 w-3" />
                              Mark
                            </>
                          )}
                        </button>
                      )}

                      <Link
                        to={`/admin/training/${a.id}`}
                        className="flex items-center gap-1 text-brand-600 hover:text-brand-800 font-medium text-sm transition"
                      >
                        <Eye className="h-4 w-4" />
                        View
                      </Link>
                    </div>
                  </td>
                </tr>
              );
            })}
          </tbody>
        </table>
      </div>
    </AdminLayout>
  );
}