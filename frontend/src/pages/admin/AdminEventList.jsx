    ...
    <AdminLayout>
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-600">{events.length} total</p>
        </div>
        <Link
            ...
            {events.length === 0 ? (
              <tr>
                <td colSpan={5} className="px-6 py-12 text-center text-slate-500">
                  No events found. <Link to="/admin/event/create" className="text-brand-600 underline">Create one</Link>
                </td>
              </tr>
            ) : (
              events.map((event) => (
                <tr key={event.id} className="hover:bg-slate-50 transition">
                  <td className="px-6 py-4 text-sm font-medium text-slate-900 max-w-md">
                  ...
                  <td className="px-6 py-4 text-sm text-slate-600 max-w-xs">
                    <div className="truncate" title={event.location || "-"}>
                      {event.location || <span className="text-slate-400">—</span>}
                    </div>
                  </td>
